...
def getCompanyFinancials(compList):
    profile_url = 'https://financialmodelingprep.com/api/v3/financials/balance-sheet-statement/'
    profile_url += compList
    print("=====>>>", profile_url, "length:", len(compList.split(',')))
    profile_details =  requests.get(profile_url )
    profile_details = json.loads(profile_details.text)
    if len(compList.split(',')) > 1 :
        profiles = profile_details['financialStatementList']
    else:
        profiles = [profile_details]

    temp_df = pd.DataFrame(columns=['symbol', 'TotalAssets', 'TotalLiabilities', 'AsOf', 'OtherAssets', 'OtherLiabilities'])
    temp_df = temp_df[0:0]
    for profile in profiles:
        Symbol = profile['symbol']
        financialValue = profile['financials'][0]
        # print("financialValue", financialValue)
        TotalAssets = float(financialValue['Total assets']) /billion
        TotalLiabilities = float(financialValue['Total liabilities']) / billion
        OtherAssets = float(financialValue['Other Assets'])/billion
        OtherLiabilities = float(financialValue['Other Liabilities'])/ billion
        # print("{}: {} | {}".format(Symbol, TotalAssets, TotalAssets))
        temp_df = temp_df.append({'symbol': Symbol,
                                                  'TotalAssets': TotalAssets,
                                                  'TotalLiabilities': TotalLiabilities,
                                                  'OtherAssets': OtherAssets,
                                                  'OtherLiabilities': OtherLiabilities}, ignore_index=True)
        # print(temp_df)
...
if __name__ == '__main__' :
    ...
    finance_df = pd.DataFrame(columns=['symbol',  'TotalAssets', 'TotalLiabilities', 'AsOf', 'OtherAssets', 'OtherLiabilities'])
    ...
    for i in range((len(shortListedCompanies) //3) +1 ) : #Should use While. as this will go extra round even if the list emptied
        ...
        if len(companyList) == 0 :
            break
        temp_df = getCompanyFinancials(companyList)
        finance_df = finance_df.append(temp_df)
    # printdf(finance_df, ['symbol',  'TotalAssets', 'TotalLiabilities', 'AsOf', 'OtherAssets', 'OtherLiabilities'])
    print("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
    main_df = main_df.set_index('symbol').join(finance_df.set_index('symbol'))
    # main_df.info(verbose=True)
    # print(main_df.head(3))
    # printdf(main_df, ['name', 'price', 'changesPercentage','dayHigh', 'yearHigh', 'priceAvg50', 'priceAvg200', 'marketCap',  'TotalAssets', 'TotalLiabilities', 'OtherAssets', 'OtherLiabilities'])
    printdf(main_df,['name', 'price', 'changesPercentage', 'yearHigh', 'marketCap', 'TotalAssets', 'TotalLiabilities', 'OtherAssets', 'OtherLiabilities'])
    # main_df.write_excel('G:\WebScrap\StockPick.xlsx')